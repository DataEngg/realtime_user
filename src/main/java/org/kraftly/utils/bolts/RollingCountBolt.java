...
import org.kraftly.utils.tools.NthLastModifiedTimeTracker;
import org.kraftly.utils.tools.SlidingWindowCounter;
...
public class RollingCountBolt extends BaseRichBolt {
    ...
    private void countObjAndAck(Tuple tuple) {
        LOG.debug("Tuple value is "+tuple.getString(0));
        LOG.debug("Tuple Type is "+tuple.getString(0).getClass().getName());
        try {
            Object obj= tuple.getString(0).split(" - - ")[0];
            counter.incrementCount(obj);
            collector.ack(tuple);
        }
        catch (Exception e){
            LOG.debug("Exception in rolling count for converting data from nginx to json");
        }
    }
